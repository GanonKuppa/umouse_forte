...
namespace robot_object{
    ...
    public:
        ...
        void updateAllOffVal(){
            ...
            ahead_off = peri::startAD_AN004() + peri::startAD_AN001();//前センサ
        };
        ...
        bool isRight(){
            ...
            int16_t threshold_right = pm.wall_threshold_right;
            if(right.at(0) > threshold_right) return true;
            else return false;
        };

        bool isLeft(){
            ParameterManager &pm = ParameterManager::getInstance();
            int16_t threshold_left = pm.wall_threshold_left;
            if(left.at(0) > threshold_left) return true;
            else return false;
        };

        bool isAhead(){
            ParameterManager &pm = ParameterManager::getInstance();
            int16_t threshold_ahead = pm.wall_threshold_ahead;
            if(ahead.at(0) > threshold_ahead) return true;
            else return false;
        };
